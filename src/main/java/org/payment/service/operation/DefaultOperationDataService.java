...
@Service
@Profile("!mock")
public class DefaultOperationDataService implements OperationDataService {
    ...
    public DefaultOperationDataService(PlatingLineElementMapper platingLineElementMapper) {
    ...
    private OperationDataSubmodelElementResponse toSubmodelElement(
        ...
        return switch (row.getPlatingElementTypeIndex()) {
            case CURRENT_TYPE_INDEX -> property("current", row.getPlatingNowValue(), "A", measuredAt);
            case VOLTAGE_TYPE_INDEX -> property("voltage", row.getPlatingNowValue(), "V", measuredAt);
